...
<!--   un main contient le traitement du formulaire de déposement d'annonce des voyages : il s'agit de recevoir les données entrées par l'utilisateur et les rangées dans la base de donnée simulées par des fichiers textes    -->
 ...
 if(isset($_FILES["urlphoto"]) && $_FILES["urlphoto"]["error"] == 0){
  ...
  if(in_array($filetype, $allowed)){
    if(file_exists("../catégories/photosvoyages/" . $_FILES["urlphoto"]["name"])){
               echo $_FILES["urlphoto"]["name"] . " existe déjà.";
             } else{
               /*    rangement des données dans le fichier texte     */
             move_uploaded_file($_FILES["urlphoto"]["tmp_name"], "../catégories/photosvoyages/" . $_FILES["urlphoto"]["name"]);
             $creatadd=fopen('../catégories/voyages.txt','a');
             $ligne=fwrite($creatadd,"\n");
            foreach ($_POST as  $value) {
              $ligne=fwrite($creatadd,$value);
              $ligne=fwrite($creatadd,' ');
            }
            $ligne=fwrite($creatadd,"photosvoyages/");
       ...
       <img class="accountcreated" src="../icons&photos/created.png"/><p class="accountcreated">   Félicitations! Votre annonce a été déposée avec succès. Vous pouvez maintenant la voir dans la catégorie Immobiliers.</p><br><br><br><?php
