...
<!--   un main contient le traitement du formulaire de création d'un contact : il s'agit de recevoir les données entrées par l'utilisateur et les rengées dans la base de donnée simulées par des fichiers textes    -->
  <main class="accountcreated">
  <?php
  $creataccount=fopen('../lesfichiersformulaires/créeruncompte.txt','a');
  $login=fopen('../lesfichiersformulaires/seconnecter.txt','a');
  foreach ($_POST as  $value) {
    $ligne=fwrite($creataccount,$value);
    $ligne=fwrite($creataccount,' ');
  }
  $lligne=fwrite($login,$_POST['email']);
  $lligne=fwrite($login,' ');
  $lligne=fwrite($login,$_POST['password']);
  $lligne=fwrite($login,' ');
   $ligne=fwrite($creataccount,"\n");
   $lligne=fwrite($login,"\n");
    $closed=fclose($creataccount);
      $lclosed=fclose($login);
   ?>
   <!--   un message affiché quand le traitement est terminé  -->
<img class="accountcreated" src="../icons&photos/created.png"/><p class="accountcreated">   Félicitations! Votre compte a été créé avec succès. Vous pouvez maintenant vous connecter pour déposer vos annonces.</p>
<br><br><br>
  </main>
  <br><br>  <br><br><br><br><br>  <br><br><br>
