...
<head>
     <meta charset="UTF-8"/>
     <title> Catégories: Téléphones </title>
...
<aside>
  <ul class="catégories">
    ...
    <li class="catégories"><a class="catégories"href="immobiliers.php"><img class="catégories" src="../icons&photos/immobiliers.png"/> Immobilier</a></li>
        <li class="catégories"><a class="actuel"  class="catégories" href="téléphones.php"> <img class="catégories"src="../icons&photos/téléphones.png"/> Téléphones</a></li>
...
<!--    un main contient les annonces déja déposées: extraites d'une base de donnée qui est simuléé  par des fichiers textes   -->
  ...
  <h2 class="téléphones"> Téléphones </h2>
  <table class="catégories">
      <!-- extraire les information de la base de donée simulé par des fichiers texte      -->
<?php
$téléphones=fopen('téléphones.txt','a+');
while(feof($téléphones)==false)
{
  ?>
    <tr class="catégories">
      <?php
  $ligne=fgets($téléphones);
  $mot="";
  $j=0;
  $length=strlen($ligne);
  for($i=0;$i<$length;$i++)
  ...
  <!--   affichage des informations extraites    -->
    <td class="img"><br>
      <img  src="<?php  echo "$colonne[4]";?>" class="img" /><br>
        </td>
        <td class="description">
          <p class="description"> <span class="téléphones"> MARQUE :</span> <?php echo "$colonne[0]";?>.</p>
            <p class="description"> <span class="téléphones"> MODELE :</span> <?php echo "$colonne[1]";?>.</p>
                    <p class="description"> <span class="téléphones"> PRIX :</span> <?php echo "$colonne[2]";?><span class="téléphones"> DA</span>.</p>
                    <p class="description"> <span class="téléphones"> NUMERO DE TELEPHONE :</span> <?php echo "$colonne[3]";?>.</p>
                    </td>
            </tr>
        <?php
}
$closed=fclose($téléphones);
?>
  </table>
<br>
  </main>
